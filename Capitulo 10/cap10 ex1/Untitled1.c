...
void Visualizar(Dados *Clientes, int pos) {
    ...
    for (i = 0; i < pos + 1; i++) {
       if(strcmp(Clientes[i].nome,n)==0){
         printf("%d",Clientes[i].num_conta);
       }
    }
...
void Excluir(Dados *Clientes, int *pos) {
    ...
    Clientes[*pos].num_conta = Clientes[pos_menor].num_conta;
    //Clientes[pos_menor].nome = Clientes[i].nome;
    Clientes[*pos].saldo = Clientes[pos_menor].saldo;
